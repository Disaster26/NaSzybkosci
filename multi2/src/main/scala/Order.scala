class Order(
  ...
  def drinksPrice: Option[Double] =
    Some(
      (for (drink <- drinks)
        yield PriceManager.drinkPrice(drink)).iterator.sum * DiscountManager.calculateDiscount(discount))

  def priceByType(pizzaType: PizzaType): Option[Double] =
    Some(
      pizzas
        .filter(pizza => pizza.pizzaType == pizzaType)
        .map(pizza => pizza.price)
        .sum * DiscountManager.calculateDiscount(discount))

  val price: Double = f"${pizzasPrice.get + drinksPrice.get}".toDouble
}
